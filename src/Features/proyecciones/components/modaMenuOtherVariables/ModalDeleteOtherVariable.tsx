...
import { deleteOtherSalario } from 'Features/proyecciones/servicios/otherVariable/delete.servicie'
import React from 'react'

export default function ModalDeleteOtherVariable(
  { data,
    AllOtherVariable
  }: any) {
  ...
  const onClickDeleteSalario = () => {
    deleteOtherSalario("proyeccion/otherVariable", data.id).then(
      (res: AxiosResponse) => {
        if (res.status === 200) {
          toast({
